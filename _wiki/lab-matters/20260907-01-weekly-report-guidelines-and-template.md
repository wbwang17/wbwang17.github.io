...
这个设计是合适的：`template` 文件和目录作为不随周次变化的底稿，每周的日期文件彼此独立，不会覆盖历史周报；公共版式和参考文献库只维护一份；`main.tex` 始终作为 Overleaf 的 Main document，每周只需修改其中一行即可切换周次。需要回看某一周时，把这一行改回对应日期的主文件即可。
